...
| `--session <id>`                        | Resume a previous session. Supports relative refs (`-1` = newest by creation time, `-2` = second-newest, … — creation order, not last-used). An explicit ID that does not exist yet is created with that ID, so a supervisor can own the session ID upfront and reuse it across runs. With `--remote`, it opens the session with that ID **on the server** — including one another client is using right now (see [Sharing a session between clients](../api-server/index.md#shared-sessions)); relative refs are local-only and are not resolved remotely. |
...
| `--remote <addr>`                       | Use a remote runtime at the given address instead of running the agent locally. Combine with `--session <id>` to open an existing server-side session, which several clients can do at once — see [Sharing a session between clients](../api-server/index.md#shared-sessions). Mutually exclusive with `--sandbox`, `--worktree`, `--worktree-pr`, `--worktree-base`, `--session-db`, `--record`, and `--fake` — a remote runtime owns its own session storage and execution environment, so these local-only concerns don't apply. |
