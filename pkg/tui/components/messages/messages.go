...
func (m *model) LoadFromSession(sess *session.Session) tea.Cmd {
	...
	// Declare toolResults early so it can be captured by closures
	toolResults := make(map[string]string)

	// collectToolResults recursively collects tool results from items (including sub-sessions)
	var collectToolResults func(items []session.Item)
	collectToolResults = func(items []session.Item) {
		for _, item := range items {
			...
			} else if item.SubSession != nil {
				collectToolResults(item.SubSession.Messages)
			}
		}
	}

	// processItems recursively processes items (including sub-sessions)
	var processItems func(items []session.Item)
	processItems = func(items []session.Item) {
		for _, item := range items {
			if item.SubSession != nil {
				// Process sub-session messages recursively
	...
	collectToolResults(sess.Messages)

	// Second pass: process all items (including sub-sessions)
